...
def save_baseline(baseline_path: str, metrics: Dict, commit: str = "unknown", notes: str = "") -> None:
...
def check_regression(current: Dict, baseline: Dict, strict: bool = False) -> Dict:
    ...
    if p95_change > 20:
        failures.append({
            ...
            'threshold': 20,
            'reason': f"p95 latency regressed by {p95_change:.1f}% (threshold: 20%)"
        })
    ...
    if p50_change > 20:
        warnings.append({
            ...
            'threshold': 20,
            'reason': f"p50 latency regressed by {p50_change:.1f}% (threshold: 20%)"
        })
    ...
    if p99_change > 25:
        warnings.append({
            ...
            'threshold': 25,
            'reason': f"p99 latency regressed by {p99_change:.1f}% (threshold: 25%)"
        })
    ...
    if error_rate_increase > 2:
        failures.append({
            ...
            'threshold': 2,
            'reason': f"Error rate increased by {error_rate_increase:.1f}% (threshold: 2%)"
        })
    ...
    if throughput_change < -30:
        warnings.append({
            ...
            'threshold': -30,
            'reason': f"Throughput decreased by {abs(throughput_change):.1f}% (threshold: 30%)"
        })
...
def format_comparison_table(current: Dict, baseline: Optional[Dict]) -> str:
    ...
    lines.append("=" * 80)
    lines.append(f"{'Metric':<20} {'Current':>12} {'Baseline':>12} {'Change':>12} {'Status':<15}")
    lines.append("-" * 80)

    # Helper function to format row
    def add_row(label: str, current_val: float, baseline_val: float, unit: str, threshold: float, reverse: bool = False):
        ...
        lines.append(
            f"{label:<20} {current_val:>11.1f}{unit} {baseline_val:>11.1f}{unit} "
    ...
    add_row("Avg Response", current['avg_response_time'], baseline_metrics['avg_response_time'], "ms", 20)
    add_row("Throughput", current['requests_per_sec'], baseline_metrics['requests_per_sec'], "/s", -30, reverse=True)

    # Error rate (absolute change, not percentage)
    error_change = current['error_rate'] - baseline_metrics['error_rate']
    error_status = "✗ FAIL" if error_change > 2 else "✓ OK" if error_change <= 0 else "↑ Worse"
    lines.append(
        f"{'Error Rate':<20} {current['error_rate']:>11.1f}% {baseline_metrics['error_rate']:>11.1f}% "
        f"{error_change:>+10.1f}% {error_status:<15}"
    )

    lines.append("=" * 80)
    lines.append(f"\nBaseline from: {baseline['timestamp']} (commit: {baseline['commit'][:7]})")
...
def main():
    ...
    parser.add_argument(
        '--update-baseline',
        action='store_true',
        help="Update baseline with current results (use after legitimate performance changes)"
    )
    parser.add_argument(
        '--strict',
        action='store_true',
    ...
    # Report results
    if result['failures']:
        print(f"\n✗ Performance regression detected ({len(result['failures'])} failures):")
        for failure in result['failures']:
            print(f"  - {failure['reason']}")

        if result['warnings']:
            print(f"\n⚠ Additional warnings ({len(result['warnings'])} warnings):")
    ...
    if result['warnings']:
        print(f"\n⚠ Performance warnings detected ({len(result['warnings'])} warnings):")
        for warning in result['warnings']:
            print(f"  - {warning['reason']}")
