"""
Alert services for evaluating drift events, delay signals, and sending
notifications.
"""
...
def evaluate_drift_event(drift_event):
    ...
    for rule in alert_rules:
        if rule.evaluate(drift_event):
            ...
            if existing:
                logger.info(
                    f"Alert event already exists for rule {rule.name} and drift event {drift_event.id}"
                )
            ...
            logger.info(
                f"Alert triggered: {rule.name} for drift event {drift_event.id}"
            )

            # Track alert creation metric
            severity_level = (
                "high"
                if drift_event.severity >= 0.7
                else "medium" if drift_event.severity >= 0.4 else "low"
            )
            track_alert_created(
                product="DriftWatch",
...
def evaluate_payment_delay_signal(payment_delay_signal):
    ...
    if (
        payment_delay_signal.severity == "low"
        and payment_delay_signal.confidence < PAYMENT_DELAY_MIN_CONFIDENCE_LOW_SEVERITY
    ):
        logger.info(
            f"Skipping low-severity, low-confidence signal for {payment_delay_signal.payer}"
        )
        return alert_events
    ...
    if existing:
        logger.info(
            f"Alert event already exists for payment delay signal {payment_delay_signal.id}"
        )
        alert_events.append(existing)
        return alert_events
    ...
    alert_rule = AlertRule.objects.filter(
        customer=payment_delay_signal.customer, enabled=True
    ).first()

    if not alert_rule:
        logger.warning(
            f"No enabled alert rules found for customer {payment_delay_signal.customer}"
        )
        return alert_events
    ...
    payload = {
        ...
        "estimated_dollars_at_risk": str(
            payment_delay_signal.estimated_dollars_at_risk
        ),
    ...
    logger.info(
        f"DelayGuard alert created for {payment_delay_signal.payer}: +{payment_delay_signal.delta_days:.1f} days"
    )
            ...
            for channel in channels:
                if channel.channel_type == "email":
                    success = send_email_notification(
                        alert_event, channel, evidence_payload
                    )
                    if success:
                        track_alert_delivered(
                            product=product_name,
                    ...
                    logger.info(
                        f"Skipping webhook channel {channel.id} - handled by send_webhooks command"
                    )
    ...
    if report_run and attach_pdf:
        try:
            ...
            artifact = ReportArtifact.objects.filter(
                customer=customer, report_run=report_run, kind="weekly_drift_summary"
            ).first()

            # Generate if missing
            if not artifact:
                logger.info(
                    f"Generating missing PDF artifact for report run {report_run.id}"
                )
            ...
            if artifact and artifact.file_path and os.path.exists(artifact.file_path):
                ...
                logger.warning(
                    f"PDF artifact file not found at {artifact.file_path if artifact else 'N/A'}"
                )
        except Exception as e:
        ...
        logger.debug(
            f"PDF attachment disabled (ALERT_ATTACH_PDF=False), skipping for report run {report_run.id}"
        )
...
def _get_suppression_reason(customer, evidence_payload):
    ...
    if recent_alert:
        logger.info(
            f"Alert suppressed: cooldown window (entity={entity_label}, signal={signal_type})"
        )
        return "cooldown"
    ...
    noise_window_start = timezone.now() - timezone.timedelta(
        days=ALERT_SUPPRESSION_NOISE_WINDOW_DAYS
    )
...
def send_slack_notification(alert_event, channel):
    ...
    if not getattr(settings, "SLACK_ENABLED", False):
        logger.info(
            f"Slack disabled (SLACK_ENABLED=False), skipping channel {channel.id}"
        )
        ...
        "blocks": [
            ...
            },
            {
                ...
                "fields": [
                    ...
                    {"type": "mrkdwn", "text": f"*Payer:*\n{payer}"},
                    {"type": "mrkdwn", "text": f"*Drift Type:*\n{drift_type}"},
    ...
    try:
        ...
        if response.status_code == 200:
            logger.info(
                f"Slack notification sent successfully for alert event {alert_event.id}"
            )
            return True
        else:
            logger.error(
...
def get_suppression_context(alert_event):
    ...
    similar_window = timezone.now() - timezone.timedelta(
        days=ALERT_HISTORICAL_CONTEXT_DAYS
    )
    similar_alerts = (
