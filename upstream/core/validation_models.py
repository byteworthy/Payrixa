...
class ValidationRule(BaseModel):
    ...
    customer = models.ForeignKey(
        "upstream.Customer", on_delete=models.CASCADE, related_name="validation_rules"
    )

    # Rule identification
    name = models.CharField(max_length=200, help_text="Human-readable rule name")
    code = models.CharField(
        max_length=50, help_text='Unique code for this rule (e.g., "REQ_001")'
    )
    ...
    execution_order = models.IntegerField(
        default=100, help_text="Order in which rules are executed (lower = earlier)"
    )
...
class ValidationResult(BaseModel):
    ...
    customer = models.ForeignKey(
        "upstream.Customer", on_delete=models.CASCADE, related_name="validation_results"
    )
    ...
    validation_rule = models.ForeignKey(
        ValidationRule, on_delete=models.CASCADE, related_name="validation_results"
    )
    row_number = models.IntegerField(
        null=True, blank=True, help_text="Row number in CSV (if applicable)"
    )
    ...
    def __str__(self):
        ...
        return (
            f"{status} - {self.validation_rule.code} - Row {self.row_number or 'N/A'}"
        )
...
class DataQualityMetric(BaseModel):
    ...
    customer = models.ForeignKey(
        "upstream.Customer", on_delete=models.CASCADE, related_name="quality_metrics"
    )
    ...
    dimension_value = models.CharField(
        max_length=255, blank=True, help_text="Value of the dimension (if applicable)"
    )

    # Metadata
    details = models.JSONField(
        default=dict, blank=True, help_text="Detailed breakdown and supporting metrics"
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                check=models.Q(sample_size__gte=0), name="dqm_sample_size_nonnegative"
            ),
            models.CheckConstraint(
                check=models.Q(passed_count__gte=0), name="dqm_passed_count_nonnegative"
            ),
            models.CheckConstraint(
                check=models.Q(failed_count__gte=0), name="dqm_failed_count_nonnegative"
            ),
            models.CheckConstraint(
...
class ClaimValidationHistory(BaseModel):
    ...
    customer = models.ForeignKey(
        "upstream.Customer", on_delete=models.CASCADE, related_name="claim_validations"
    )
    claim_record = models.ForeignKey(
        "upstream.ClaimRecord", on_delete=models.CASCADE, related_name="validations"
    )
    upload = models.ForeignKey(
        "upstream.Upload", on_delete=models.CASCADE, related_name="claim_validations"
    )
...
class DataAnomalyDetection(BaseModel):
    ...
    customer = models.ForeignKey(
        "upstream.Customer", on_delete=models.CASCADE, related_name="data_anomalies"
    )
    upload = models.ForeignKey(
        "upstream.Upload",
        on_delete=models.CASCADE,
    ...
    statistical_details = models.JSONField(
        default=dict, help_text="Statistical details: mean, std, z-score, etc."
    )
    ...
    def __str__(self):
        return (
            f"{self.anomaly_type} - {self.severity} - Score: {self.anomaly_score:.2f}"
        )
