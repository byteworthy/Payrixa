...
def data_quality_dashboard(request: HttpRequest) -> HttpResponse:
    ...
    # Get recent uploads
    # HIGH-13: Add prefetch_related to avoid N+1 queries when accessing quality_report
    recent_uploads = (
        Upload.objects.filter(customer=customer)
        .prefetch_related("quality_report")
    ...
    recent_anomalies = DataAnomalyDetection.objects.filter(
        customer=customer, acknowledged=False
...
def validation_rules_dashboard(request):
    ...
    recent_failures = (
        ValidationResult.objects.filter(customer=customer, passed=False)
...
def quality_trends(request):
    ...
    metrics_with_json = [
        {"type": metric_type, "data": data, "data_json": mark_safe(json.dumps(data))}
...
def acknowledge_anomaly(request, anomaly_id):
    ...
    customer = request.user.profile.customer
    anomaly = get_object_or_404(DataAnomalyDetection, id=anomaly_id, customer=customer)
...
def quality_metrics_chart_data(request):
    ...
    metrics = DataQualityMetric.objects.filter(
        customer=customer, metric_type=metric_type, measurement_date__gte=start_date
...
def validation_failure_chart_data(request):
    ...
    failures = (
        ValidationResult.objects.filter(
            customer=customer, passed=False, created_at__gte=start_date
        )
        .values("validation_rule__rule_type")
